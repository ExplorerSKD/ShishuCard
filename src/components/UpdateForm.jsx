...
const VaccineModal = ({ isOpen, onClose, onSubmit, childData }) => {
  const [selectedVaccines, setSelectedVaccines] = useState(() => {
    // Handle case where childData or vaccinations might not exist
    if (!childData) return [];
    
    // If childData has vaccinations, use those
    if (childData.vaccinations && childData.vaccinations.length > 0) {
      return childData.vaccinations.map(v => ({
        ...v,
        done: v.done || false,
        recommendedAge: v.recommendedAge || v.age || "Not specified"
      }));
    }
    
    // Otherwise, use the default vaccine schedule from JSON
    return vaccine.map(v => ({
      name: v.name,
      done: v.status === 'completed',
      recommendedAge: v.maxAge ? `${v.maxAge} weeks` : "Not specified",
      dueDate: v.dueDate,
      status: v.status
    }));
  });
          ...
          <div>
            ...
            {childData && (
              <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">
                For: {childData.name || childData.childName} (ID: {childData._id || childData.childId})
              </p>
            )}
          </div>
