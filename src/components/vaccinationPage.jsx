...
useEffect(() => {
  ...
  const saveTodbAndGenerateQR = async () => {
    const childData = {
      ...
      address: {
        street: formData.birthplace,
        city: formData.hospitalName
      },
      // Store vaccination data as additional info for now
      medicalHistory: [{
        condition: 'Initial Vaccination Schedule',
        diagnosis: new Date(),
        notes: JSON.stringify(vaccinations)
      }],
      // Parent information (can be expanded)
      parentInfo: {
    ...
    try {
      ...
      const savedChild = response.data.data;
      
      console.log("Data saved successfully!", savedChild);
      setChildId(savedChild._id);
      setQrValue(JSON.stringify({
        childId: savedChild._id,
        name: savedChild.name,
        dateOfBirth: savedChild.dateOfBirth,
        vaccinations
      }));
      
      toast.success('Child registration successful!');
    } catch (error) {
      console.error("Error saving data:", error);
      toast.error('Failed to register child. Please try again.');
      return; // Don't send email if registration failed
    }
